...
4. The global agent surface under `agents/` — canonical skills grouped by category (`agents/skills/<category>/<name>`; skill names globally unique), retired skills under `agents/archive/`, global agent instructions (`agents/AGENTS.global.md`), and the `skillctl` invocation-state tool. Not a stow package: `setup/agents.sh` owns machine-level wiring and projects each canonical skill as a flat `~/.claude/skills/<name>` symlink because Claude Code does not discover the categorized intermediate directory; `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` both point at `AGENTS.global.md`; Codex preserves the categorized hierarchy through per-skill symlinks refreshed by the `skillctl sync` call inside setup so Codex-managed `.system/` survives. Existing same-name Claude skills are not overwritten. See `agents/README.md`.
