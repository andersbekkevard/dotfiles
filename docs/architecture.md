...
4. The global agent surface under `agents/` contains canonical active skills in the flat `agents/skills/<name>` namespace, candidates under `agents/in-progress/<name>` or `agents/in-progress/<collection>/<name>`, and retired skills under `agents/archive/<name>`. Reusable skills remain public. Anders-specific operational skills may occupy the same flat namespace while git-crypt protects their entire directory. A locked clone skips those ciphertext `SKILL.md` files and removes their managed harness links; unlocking makes them available without a second repository or a second skill namespace. Encrypted real-use evidence lives separately under `agents/skill-uses/<skill-name>/<UTC timestamp>/`; it informs later revision without entering normal skill context. Cheap usage telemetry has a separate owner: `agents/skill-usage` keeps atomic machine-local state in SQLite and publishes encrypted immutable delta batches under `agents/skill-usage-batches/<replica>/<sequence>-<hash>.json`. Global totals are derived by summing batches, never stored in a shared mutable counter. Collection directories under `in-progress/` contain no `SKILL.md` and exist only to organize related experiments. The surface also contains primary shared global instructions in `agents/SHARED.global.md`, harness additions in `agents/AGENTS.global.md` and `agents/CLAUDE.global.md`, and optional Git-ignored machine overlays under `agents/.local/`. This is not a stow package. `setup/agents.sh` owns machine-level wiring, composes tracked shared and harness instructions followed by local shared and harness overlays into `~/.codex/AGENTS.md` and `~/.claude/CLAUDE.md`, projects only available active skills as flat per-skill symlinks for both harnesses, and initializes the local telemetry baseline once per machine. `agents/instructionctl` inspects and verifies composed files without writing them. The `skillctl sync` call inside setup refreshes Codex links without touching Codex-managed entries such as `.system/`. Existing same-name skills are not overwritten. See `agents/README.md`.
5. Retired configs and scripts under `archive/`.

The shell startup boundary clears an inherited `NO_COLOR` only for Ghostty.
macOS can register a Ghostty application job with the environment of the GUI
process that first launched it, and later Finder launches can reuse that job
while the Ghostty process remains alive. Ghostty supplies the correct terminal
capability variables to its child shell, so a launcher-level monochrome request
must not override them there.
...
Fleet's git-crypt-protected registry and verified SSH host identities live under
...
key fails closed without altering the user's general SSH trust store. A locked
clone reports that the private Fleet configuration must be unlocked or replaced
through `FLEET_CONFIG` and `FLEET_KNOWN_HOSTS`. Fresh
