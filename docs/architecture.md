...
4. The global agent surface under `agents/` — canonical skills grouped by category (`agents/skills/<category>/<name>`; skill names globally unique), retired skills under `agents/archive/`, primary shared global instructions (`agents/SHARED.global.md`), harness additions (`agents/AGENTS.global.md` and `agents/CLAUDE.global.md`), optional Git-ignored machine overlays under `agents/.local/`, and the `skillctl` invocation-state tool. Not a stow package: `setup/agents.sh` owns machine-level wiring, composes tracked shared and harness instructions followed by local shared and harness overlays into `~/.codex/AGENTS.md` and `~/.claude/CLAUDE.md`, and projects each canonical skill as a flat `~/.claude/skills/<name>` symlink because Claude Code does not discover the categorized intermediate directory. Codex preserves the categorized hierarchy through per-skill symlinks refreshed by the `skillctl sync` call inside setup so Codex-managed `.system/` survives. Existing same-name Claude skills are not overwritten. See `agents/README.md`.
