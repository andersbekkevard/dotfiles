...
- The full layer installs or updates Claude Code and the Codex CLI with their official standalone installers. Setup puts `~/.local/bin` on `PATH` before invoking either installer so they do not write machine-specific PATH blocks through the stowed shell profile. This makes both `claude` and `codex` part of the verified `full`, `macos`, and `linux-desktop` machine contracts without coupling their updates to the Node package manager.
