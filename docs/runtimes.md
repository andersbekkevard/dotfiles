...
- agent-browser: latest pnpm global release for the Linux desktop profile. Its stable `~/.local/bin/agent-browser` launcher puts daemon sockets under `/tmp/agent-browser-<uid>` so workspace-sandboxed Codex tasks do not need approval to write under `~/.agent-browser`.
...
- The Linux desktop layer installs and updates `agent-browser` through pnpm. Its generated stable launcher sets `AGENT_BROWSER_SOCKET_DIR` only when the caller has not already selected one, preserving explicit overrides while keeping the default daemon socket inside the sandbox-writable `/tmp` boundary.
