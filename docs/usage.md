...
env -i HOME="$HOME" USER="$USER" SHELL=/bin/zsh PATH=/usr/bin:/bin:/usr/sbin:/sbin \
  zsh -lc 'command -v git nvim ngrok cloudflared delta trufflehog fleet fnm node pnpm cargo bun tree-sitter claude codex psql typescript-language-server'
```
...
env -i HOME="$HOME" USER="$USER" PATH="$HOME/.local/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
  sh -lc 'command -v git nvim ngrok cloudflared delta trufflehog fleet fnm node pnpm cargo bun tree-sitter claude codex psql typescript-language-server wt'
```
