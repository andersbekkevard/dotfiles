...
This skips package/runtime installers but still runs the repo-managed agent work: minimal stow packages, local templates, flat `~/.claude/skills/<name>` links, composed `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` files, and Codex-generated categorized skill state. The global instruction files are composed from primary `agents/SHARED.global.md` content followed by `agents/CLAUDE.global.md` or `agents/AGENTS.global.md`. On a fresh machine that still needs packages, run the normal explicit profile instead.
