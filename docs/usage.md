...
This skips package/runtime installers but still runs the repo-managed link work: minimal stow packages, local templates, `~/.claude/skills/<name>`, `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, and Codex-generated skill state. On a fresh machine that still needs packages, run the normal explicit profile instead.

Day-to-day invocation-mode work goes through `skillctl`:

```bash
agents/skillctl list                    # every skill: mode, always-loaded token cost, description
agents/skillctl disable-model <skill>   # flip model-invoked -> user-invoked
...
Invariants: `setup/agents.sh` is the machine-level owner for global agent links; direct `agents/skillctl sync` is not a substitute for setup because it does not create Claude per-skill links or top-level harness instruction links; SKILL.md frontmatter is the single source of truth for invocation mode; `agents/openai.yaml` policy blocks and `~/.codex/skills/*` symlinks are generated by `sync` and never hand-edited; `setup/agents.sh` maintains `~/.claude/skills/<name>` as per-skill symlinks so user or third-party Claude skills can coexist; same-name Claude skill conflicts are skipped with a warning, not overwritten; `agents/skill-sources.toml` is the single source of truth for global skill provenance and upstream drift checks; repo-specific skills live in each repo's `.agents/skills/`, not here. Details: `agents/README.md`.
