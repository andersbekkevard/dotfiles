...
Global agent skills and instructions live under `agents/` and are linked into `~/.claude` and `~/.codex` by the minimal layer (`setup/agents.sh`). Day-to-day invocation-mode work goes through `skillctl`:
...
agents/skillctl sync                    # regenerate Codex yaml + symlinks; idempotent
