...
Invariants: SKILL.md frontmatter is the single source of truth for invocation mode; `agents/openai.yaml` policy blocks and `~/.codex/skills/*` symlinks are generated by `sync` and never hand-edited; repo-specific skills live in each repo's `.agents/skills/`, not here. Details: `agents/README.md`.
