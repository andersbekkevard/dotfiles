...
Status: work in progress, tournament candidate. Revision of `html-fable` after
a 2026-08-21 blind-run evaluation (3 scenarios, Opus subagents, 34/34 objective
style checks): scoped `page.css` flow selectors under `svg.flow` so Mermaid's
generated `.node`/`.edge` classes keep their theme, and gave `svg.flow` a
`min-width` so wide figures scroll inside their canvas at narrow widths instead
of shrinking their labels — both defects surfaced by the blind runs.
