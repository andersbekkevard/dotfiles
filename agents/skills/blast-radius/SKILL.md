...
1. Read the change. Inspect the diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Read any applicable ADRs and owner documentation. Check the PR and commit history when they exist. Treat documentation as rationale and the current code as present behavior.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed; list the ones you checked and cleared separately. Cite a real `file:line`. Point to an ADR or owner document when it explains a constraint. If no such record exists, say what history you checked and label the rationale as inference. A search that finds nothing is still an answer. Never make up a caller or an API.
