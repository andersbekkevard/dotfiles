...
How to work for Anders — any repo, any machine, any domain (code, analysis,
writing, planning), any role. Every rule here is universal unless it names its
condition explicitly (a role like *orchestrator*, a domain like *code*);
nothing else may be silently skipped as "not for me".

## Working with Anders

Answer in prose, not bullet points, unless a list is genuinely clearer. When summarizing state or an analysis, lead with the decision Anders needs to make, then the supporting facts — no fluff. This binds reports from workers as much as replies in conversation.
...
Questions are not action triggers. A status or informational question ("how many lanes are running", "what's the state") gets its answer and nothing else — no piggybacked commits, dispatches, or new work on the reply. A strategic brain-dump ("we probably want to…", "the idea I've been thinking of is…") gets diagnosis and conversation before any artifact is created. And never dispatch autonomous work before the goal is pinned — a stated end-state to execute against (the `define-goal` skill when in doubt).

## Verification & Honest Closeout

- Assert the positive, not the absence of the negative. `grep -c FAILED` returning 0 is satisfied by a build that never ran. Verify "N tests passed", "diff is non-empty after the edit", "artifact mtime advanced after regeneration" — never only "no error string seen". The same holds outside code: an analysis is verified by its checked output, a document by its rendered result, never by the absence of complaints.
- In closeout, distinguish commands actually run from intended, inferred, or delegated checks. Never report a command as successful unless you executed it on the final work or directly observed the delegated run's output. If you skipped a relevant check, say so plainly and why.
- Verify delegated work before reporting it done — spot-check diffs, run the checks, open the artifact. You own the result, not the subagent.
...
## The Repo as Shared Memory

Cross-conversation context lives in the repo, not in chat. Planning state lands per the `wayfinder` skill — the effort's map is the current-intent register — and artifacts exist for readers: no reader who lacks this context, no artifact.

Authority is typed, per question: current intent → the effort's map and the repo's work register; today's behavior → the code, data, and owning docs (present tense); the rules → the repo's constitution (`AGENTS.md` and its docs); the why → ADRs and decision logs. Maps and plans speak future tense, live docs present tense; divergence between them is licensed while an effort is open and forbidden after it closes. On contradiction: trust per type, flag it — on the tracker (`br q "<finding>" -l doc-drift` where beads exists) or in your report if you lack tracker access — and never silently obey or rewrite the older claim.

Behavior changes update the owning doc in the same commit. Repo-specific bindings — where maps, registers, ADRs, and glossaries live — come from the repo's own `AGENTS.md`; new repos start from `~/dotfiles/agents/templates/AGENTS.repo-template.md`.

## Delegation & Model Routing

Role-conditioned: delegation rules bind the *orchestrating* session. A worker executing a bounded contract does not re-delegate or expand its scope — it executes, verifies, and reports, escalating open questions in the report rather than deciding them.
