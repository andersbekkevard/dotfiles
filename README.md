...
The shared base layer installs the same core CLI set on every machine, including `cloudflared`, `ngrok`, `git-delta`, and the `git-loc` remote repository line-count helper. `cloudflared` is the canonical path for publishing local services on Anders' Cloudflare-managed domains; the global `personal-edge` skill owns the agent workflow. The full profile adds the Codex CLI, CLIProxyAPI, the isolated `claudex` Claude-Code-with-Codex entrypoint, `git-crypt`, and developer tools including the PostgreSQL client (`psql`) required by Neovim Dadbod for PostgreSQL connections.
