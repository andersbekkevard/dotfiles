...
Maintain a one-hit, idempotent, cross-platform dotfiles bootstrap (`init.sh`) with predictable profile behavior and verifiable outcomes.
...
3. `docs/profiles.md` (profile semantics and auto-detection)
...
| `docs/profiles.md` | Profile behavior | `auto` resolution and profile boundaries |
...
- **CLI flow changes** (`init.sh` flags, verify behavior) → update `docs/usage.md`
...
- Keep terminology stable (`minimal`, `full`, `macos`, `linux-headless`, `linux-desktop`, `auto`).
